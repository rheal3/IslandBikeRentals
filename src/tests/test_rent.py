...
class TestRent(unittest.TestCase):
    ...
    def test_booking_create(self):
        ...
        # check if booking with email in booking table
        booking = Booking.query.filter_by(email=booking_data["email"]).first()

        # check the OK status
        self.assertEqual(response.status_code, 302)

        # test there's data in the response
    ...
    def test_booking_payment(self):
        ...
        response = self.client.post("rent/", data=booking_data)
        booking_id = Booking.query.filter_by(email=booking_data["email"]
                                             ).first().id
        ...
        # test html contains changed data from updated booking
        self.assertEqual(payment_data["full_amount_due"],
                         payment.full_amount_due)
        self.assertEqual(payment_data["upfront_amount_paid"],
                         payment.upfront_amount_paid)
